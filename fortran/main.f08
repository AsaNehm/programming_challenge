...
real(kind=8) :: tmp_sum
        ...
        !$omp parallel &
        !$omp default(shared) &
        !$omp private(destination_i,k,tmp_outdeg,source_i,i)
        ...
        DO WHILE (elapsed .LT. MAX_TIME .AND. (elapsed + time_per_iteration) .LT. MAX_TIME)
            ...
            DO source_i = 0, GRAPH_ORDER - 1
                new_pagerank(source_i) = 0.0
                DO destination_i = 0, GRAPH_ORDER - source_i - 1
                    IF (adjacency_matrix(destination_i, source_i) .EQ. 1.0) THEN
                        new_pagerank(source_i) = new_pagerank(source_i) + pagerank(destination_i) * outdeg_div_arr(destination_i)
                    END IF
                END DO
                new_pagerank(source_i) = DAMPING_FACTOR * new_pagerank(source_i) + damping_value
            END DO


            !$omp do schedule(dynamic) reduction(+:diff)
            DO i = 0, GRAPH_ORDER - 1
            ...
            !$omp do schedule(static) reduction(+:pagerank_total)
            DO i = 0, GRAPH_ORDER - 1
